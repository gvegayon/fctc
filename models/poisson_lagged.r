...
for (Wnum in 1:nrow(networks)) {
  ...
  for (m in seq_along(models)) {
    ...
    for (art in articles) {
      ...
      # Run the model
      ans <- tryCatch(stats::glm(mod, data=tmpdata, family = poisson()), error=function(e) e)
      ...
      # Creating the object
      assign(paste("poisson_lagged",art, m, sep="_"), ans, envir = .GlobalEnv)
  ...
  # Saving results -----------------------------------------------------------
  save(Wname, list = ls(pattern = "poisson_lagged_sum.+[0-9]$"), file = sprintf("models/poisson_lagged_%s.rda", Wname))
...
save(models, networks, articles,
     file = sprintf("models/poisson_lagged_specifications.rda"))
