...
for (Wnum in 1:nrow(networks)) {
  ...
  for (art in articles) {
    ...
    # Creating the object
    assign(paste("ols_lagged",art,1,sep="_"), ans, envir = .GlobalEnv)
  ...
  for (art in articles) {
    ...
    # Creating the object
    assign(paste("ols_lagged",art,2,sep="_"), ans, envir = .GlobalEnv)
  ...
  for (art in articles) {
    ...
    # Creating the object
    assign(paste("ols_lagged",art,3,sep="_"), ans, envir = .GlobalEnv)
  ...
  for (art in articles) {
    ...
    # Creating the object
    assign(paste("ols_lagged",art,4,sep="_"), ans, envir = .GlobalEnv)
  ...
  for (art in articles) {
    ...
    # Creating the object
    assign(paste("ols_lagged",art,5,sep="_"), ans, envir = .GlobalEnv)
  ...
  for (art in articles) {
    ...
    # Creating the object
    assign(paste("ols_lagged",art,6,sep="_"), ans, envir = .GlobalEnv)
    message("Network ", Wname, " article ", art, " model ", 6,  " done.")
  }
  
  
  # Saving results -----------------------------------------------------------
  save(Wname, list = ls(pattern = "ols_lagged_sum.+[0-9]$"), file = sprintf("models/ols_lagged_%s.rda", Wname))
