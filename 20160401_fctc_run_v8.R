...
# [2016-04-01]: You missed a comma here
...
# [2016-04-02] There is no variable 'threshold6' in ftct_att, the data.frame
#   was named like that, but the actual variable was called 'thr6'.
# mod_all <- as.formula(paste(
#   # Threshold regression
#   "threshold6 ~ factor(year) + factor(continent) +
#   population + democracy + GL_total + tobac_prod +
#   degree_01 + degree_02 + degree_03 + degree_12 + degree_13 + degree_14 "
# )
# )
...
imp_mod_all <- as.formula(paste(
  "threshold ~ factor(continent) + toa_year_fctc +
  population + democracy + GL_total + tobac_prod +
  degree_01 + degree_02 + degree_03 + degree_12 + degree_13 + degree_14 +
  cohexp_01 + cohexp_02 + cohexp_03 + cohexp_12 + cohexp_13 + cohexp_14 "
)
...
imp_mod_all <- as.formula(paste(
  "meanall ~ factor(who_reg) + toa_year_fctc + perc_male_smoke + labor +
  population + democracy + GL_total + tobac_prod +
  degree_01 + degree_02 + degree_03 + degree_12 + degree_13 + degree_14"
)
)
...
# summary(imp_out_all)
#
# imp_out_all <- glm(imp_mod_all, data=fctc_att, family = gaussian(link="binomial"),
#                    subset = ((year ==  10) & (degree_14>1)) & (id!=183) & (id!=163))
# summary(imp_out_all)
