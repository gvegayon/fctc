...
library(parallel)
cl <- makeCluster(10)
clusterExport(cl, c("model_data"))
clusterEvalQ(cl, library(netdiffuseR))

# Distance network -------------------------------------------------------------
for (Wname in c("adjmat_mindist")) {
  ...
  for (art in articles) {
    ...
    ans <- struct_test(W, function(w) {
      coef(lagsarlm(mod, data = model_data, listw = mat2listw(w, style="W"),zero.policy = TRUE))
      }
      cl=cl, parallel="multicore", ncpus=length(cl), R=20)
    stop()
    # Creating the object
