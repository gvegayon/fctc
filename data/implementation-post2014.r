...
# Parameters
codes       <- read.csv("data-raw/fctc_implementation_2014-and-on/key.csv")
...
for (article in articles) {
  ...
  ans <- list()
  for (ind in ss$id) {
    dat  <- read.csv(sprintf("data-raw/fctc_implementation_2014-and-on/%s.csv", ind))
    ...
    if (max(lvls[[as.character(ind)]]) > 10) next
  ...
  # Saving the object
  assign(sprintf("art%05s", article), newdat)
  
  message("Article ", article, " processed.")
}

# Putting all together
arts <- ls(pattern = "^art([0-9]|\\s)")
...
dat[dat$country_name=="Libya",]$entry <- "LY"
...
implementation <- subset(implementation, select = c(
  "entry", "year", "no_report", sprintf("sum_art%05s", art_to_keep)
))

colnames(implementation) <-
  c("entry", "year", "no_report", sprintf("sum_art%02d", as.integer(art_to_keep)))

write.csv(implementation, file = "data/implementation-post2014.csv", row.names = FALSE, na="<NA>")
