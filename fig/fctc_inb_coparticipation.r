...
W   <- adjmat_fctc_inb_coparticipation_twomode
...
put_together <- function(x, net, criter = 30) {
...
for (yearnum in c(2010, 2012)) {
  ...
  # Making space
  pdf(sprintf("fig/fctc_inb_coparticipation%i.pdf", yearnum),
      width = 10, height = 12)
  ...
  for (art in c(5, 6, 8, 11, 13)) {
    ...
    vsize <- rescale_vertex_igraph(degree(net), minmax.relative.size = c(.01, .05))
    lsize <- (vsize - min(vsize))/(max(vsize) - min(vsize)) + .5
